...
def calculate_iou(boxes1, boxes2):
    """
    Calculate the Intersection over Union (IoU) of two sets of bounding boxes using PyTorch.
    """
...
def calculate_average_precision(recalls, precisions):
    """
    Calculate the average precision (AP) given recall and precision curves.
    """
...
def calculate_MAP(predicted_boxes, ground_truth_boxes):
    r"""
    Calculate the mean average precision for bounding box detection.
    """
...
class BoundingBoxEvalCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        # Prepare lists for predictions and targets in torchmetrics format.
        pred_list = []
        target_list = []
        dummy_label = 1  # Using a single class label for all boxes
        
        for pred, target in zip(predictions, self.val_dataset):
            # -------------------------------
            # Parse predicted bounding boxes
            # -------------------------------
            # Assuming pred is a string like "10,10,50,50; 12,14,48,52"
            if isinstance(pred, str):
                box_list = []
                for box_str in pred.split(';'):
                ...
                # Convert list to tensor. If no boxes, create an empty tensor.
                boxes = torch.tensor(box_list) if box_list else torch.empty((0, 4))
            else:
                # If pred is already a dict with "boxes", use it directly.
                boxes = torch.tensor(pred.get("boxes", [])) if isinstance(pred, dict) else torch.empty((0, 4))
            
            pred_dict = {
                "boxes": boxes,
                "scores": torch.ones(boxes.shape[0]),  # Assign constant score 1.0 to all predictions.
            ...
            # -------------------------------
            # Expecting target to be a dict with key "boxes"
        ...
        # Initialize the MeanAveragePrecision metric from torchmetrics
